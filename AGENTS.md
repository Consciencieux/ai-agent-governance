...
- Distribution roles (use these three names, never the bare word "payload" — it used to mean all three and that ambiguity produced real defects): **INSTALLED** = INIT writes it into the governed project (it appears as a `source` in `references/init-spec.json`); **SKILL-INTERNAL** = travels in the tarball and the skill executor reads it, but INIT never installs it, so a governed project does NOT have it (`references/init-spec.json`, `references/workflows/release.md`, `scripts/generate-governance.js`, `scripts/check-coding-hygiene.js`, `scripts/check-doc-parity.js`, `scripts/check-layout-sync.js`, `scripts/check-plan-delivery.js`, `scripts/check-role-completeness.js`, `scripts/package-skill.sh`); **REPO-ONLY** = never in the tarball (`docs/`, `tests/`, `package.json`, `.github/`, README, CONTRIBUTING, CHANGELOG, AGENTS.md, `.gitattributes`). Two consequences: a SKILL-INTERNAL file must never be cited as a rule source for governed projects, and a SKILL-INTERNAL script must no-op outside this repo's shape. Full table: `docs/en/architecture.md` § Three distribution roles
