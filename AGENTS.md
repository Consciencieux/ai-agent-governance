...
| Agent instruction architecture (thin entrypoint, contextual loading, mechanical-first) | `docs/design-decisions/ADR-0022` · `docs/research/RESEARCH-0009` · this file § Conventions | both |
...
- **CHANGELOG format is unified** — version heading (`## [X.Y.Z] - date` / `## [Unreleased]`) followed by a blank line; category headings (`### <canonical name>`) surrounded by blank lines; list items separated by blank lines. No compact style (`##`→`###`→item without blanks). The same rule governs this repo's own CHANGELOG and governed projects (authority: `references/policies/lifecycle.policy.md` § CHANGELOG 结构契约 格式统一); mechanically enforced by the `changelogCoverage` cluster in `scripts/check-doc-consistency.js` on the newest section, fail-closed under `--gate`/`--release-gate` (the 32-section audit reformatted all of them once; the gate now keeps the section being edited in the single style).
- **Repo-owned accession policy (ADR-0012 § 后续补充): decision ≠ delivered change.** `ADR Accepted` / new Finding / new Research / Plan creation do NOT by themselves produce a CHANGELOG entry. Enter CHANGELOG only when observable project behavior, public interface, contributor workflow, release behavior or supported capability actually changed. `[Unreleased]` = implemented changes ready for the next release — not all discussed work, not every Accepted ADR, not every Active Plan, and not a per-migration-commit log (Migration Mode forbids releases, so compose `[Unreleased]` at checkpoint/merge/release boundaries). Released sections are historical records, default immutable (factual corrections and authorized history cleanup allowed; rewriting the past to match today's architecture is not). This accession/`[Unreleased]`/released-section policy is owned by THIS file (the Repo Profile); the payload `references/policies/lifecycle.policy.md` remains authoritative for the shared format contract governing governed projects, which this repo consumes.
- Plans are design docs in `docs/plans/` (single-language, zh-CN canonical); completed plans are archived to `docs/plans/archive/` at release, never deleted. A new plan is written once in `docs/plans/`; on archive it moves to `docs/plans/archive/`.
...
Run the gate group before declaring any task done; run the full group (`npm run check:all`) before audit, and the release-specific gate (`npm run check:skill-release`, which adds the `--release-gate` fail-closed clusters) before release per `repo-workflows/skill-release.md`. Record real output (never claim "should pass").
...
- Language policy by audience: agent-facing files (`SKILL.md`, `references/**`, generated artifact bodies) are single-language - never add a second language section. Within `docs/`, language follows knowledge type (not the whole tree): **user-facing product docs** are trilingual and split — the root keeps only the English landing files (`README.md`, `CONTRIBUTING.md`), translations live in their trees (`docs/product/zh-CN/`, `docs/product/zh-TW/`), and `docs/product/en/` holds the rest of the English docs; **roadmap** (`docs/plans/roadmap/`) is trilingual; **plans / findings / research / design-decisions** are 简体中文 canonical single-language (shared); archived plans (`docs/plans/archive/`) are shared single-language 简体中文. 简体中文 is the canonical source for all trilingual docs; editing one language requires updating the other two in the same change. New terms must be added to `docs/glossary.md` first
