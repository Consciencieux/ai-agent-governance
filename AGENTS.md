...
| Two-domain symmetry (governance defect cross-domain check) | `references/policies/lifecycle.policy.md` § 根因修复协议与失败预算 | payload |
...
- Plans are design docs in each language tree's `plans/` (`docs/<lang>/plans/`); completed plans are archived to `docs/archive/` (shared, single-language) at release, never deleted. **A new plan must be written in all three trees in the same change** (`docs/zh-CN/plans/` + `docs/en/plans/` + `docs/zh-TW/plans/`, same filename, zh-CN canonical; structural parity is enforced by `check-doc-parity.js`, so a one-tree plan fails CI). On archive, the trilingual copies are removed and the single-language original goes to `docs/archive/`.
...
- **Roadmap is an index, not a fact source** — the design plans (`docs/{en,zh-CN,zh-TW}/plans/`) are the single source of truth for a feature's design, state and acceptance criteria; the roadmap lists a horizon classification and a one-line summary per future item, and every future item links its design plan (or explicitly says *no design plan yet*). A roadmap entry must NEVER restate a plan's detail (validation method, Affected Files, steps) — that is duplicated authority. When a plan is created, archived, or implemented, the roadmap entry is updated in the SAME change: implemented plans move to Done (linking the archive), archived/withdrawn plans are removed from the active horizon, and new plans get a link in their horizon if that horizon is non-empty. Roadmap re-baseline happens at each release per the maintenance rule in `docs/en/roadmap.md`, but **item-level sync happens at plan lifecycle events, not only at release** — an implemented feature that never reached the Done list is a governance defect (the release gates do not catch it: gates verify declarations, not roadmap completeness).
