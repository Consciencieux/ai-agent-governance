...
- **Repo-owned accession policy (ADR-0012 § 后续补充): decision ≠ delivered change.** `ADR Accepted` / new Finding / new Research / Plan creation do NOT by themselves produce a CHANGELOG entry. Full repo CHANGELOG policy (accession test, `[Unreleased]` semantics, released-section immutability, classification) is REPO-ONLY in `repo-workflows/changelog-policy.md` — read it when writing CHANGELOG; the payload `references/policies/lifecycle.policy.md` remains authoritative for the shared format contract governing governed projects.
