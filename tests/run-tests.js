...
// Structure (anti-patch plan §3, batch 1): this file keeps the runner + ALL shared
// helpers (same shared scope as the former single file); tests live in tests/suites/*.
// Batch 2 will consolidate the shared helpers into tests/support/helpers.js requires.
...
// shared helpers exposed on global (batch 1 preserves the original shared scope; batch 2
// consolidates into tests/support/helpers.js requires). Mirrors pre-split behavior.
for (const n of ["tmp","write","assemble","run","cleanup","buildFullDefault","buildParityTrees","gitCommitAt","buildFreshnessFixture","runRelease","planChanges","buildI18nFixture","gitInit","gitHead","gitTags","listFiles","buildLayoutRepo","buildPlanRepo","findPosixShell","copiedScriptSources","writeConsentSyncPoint","linkDir"]) {
  if (typeof global[n] === "undefined") global[n] = eval(n);
}
...
for (const n of ["VALIDATOR","LOCK_CHECK","GIT_POLICY_CHECK","SECRET_CHECK","SYNC_CHECK","GENERATOR","LAYOUT_CHECK","PLAN_DELIVERY","TMP_ROOT","PARITY_CHECK","FRESHNESS_CHECK","CONSISTENCY_CHECK","RELEASE_TOOL","SKILL_ROOT","CONSISTENCY","CONSENT_THREE_MARKERS_TEXT","TMP_ROOT"]) {
  if (typeof global[n] === "undefined") global[n] = eval(n);
}
