...
- **3a. Generated-references-resolve check**: after INIT in the existing payload fixture framework, verify every executable instruction in the generated sub-skills (`docs/rules/*.md`, `AGENTS.md`, `.governance/generated/skills/**`) resolves to a file present in the generated project. To avoid guessing at prose, an executable instruction is defined as one of exactly two shapes: a sentence beginning with `Run:` / `运行：` followed by a command, or a line of fenced code that starts with `node scripts/...`, `bash scripts/...` or an npm script name. NOT judged: explanatory mentions ("the installed `scripts/check-sync.js`"), provenance parentheticals ("derived from `references/init-spec.json`"), conditional descriptions ("if the project has such trees..."), or code samples that are not commands. This is the minimal grammar review point 5 asks for; everything else stays an unresolvable judgement about prose. Extend the existing payload test `INIT installs the release tag executor the sub-skill invokes` (already does this for release-manager) to cover all generated sub-skills and the review/drift flows, and the AGENTS.md + docs/rules files.
...
- **3c. Dead-link sweep — placed in the payload fixture, NOT in check-doc-consistency.js.** The frozen-responsibility bar (consistency § FROZEN) says new checks only land in that script when they are cross-document fact clusters; target-runtime portability is not one. The correct home is the payload fixture: after INIT, resolve every relative markdown link in the generated AGENTS.md and docs/rules/*.md and flag one that leads to a path outside the generated project. This stays in `tests/suites/payload.test.js` (mechanical, per-fixture); a production gate for this would be a new dedicated script with a clear name (`check-payload-portability.js`), NOT an extension of `check-doc-consistency.js`. If the fixture check starts producing false positives (e.g. a target rule intentionally pointing at `docs/`), reduce it to advisory-notice level rather than moving it into a gate.

**Decision — the evidence form for reference closure is an audit obligation plus clean-target verification, NOT a full mechanical gate.** Recorded here rather than as an ADR: the closure principle itself has no defensible alternative (nobody argues references need not resolve), so it is a principle, not a decision. What WAS a decision is how it gets enforced, and it has a real rejected option.

- **Rejected: a general closure gate** that walks every reference in every payload file and fails when one does not resolve in a governed project. It cannot distinguish an executable instruction ("run `scripts/check-sync.js`") from provenance prose ("derived from `references/init-spec.json`") or a deliberate skill-repo statement inside a SKILL-INTERNAL file. Every one of those is a legitimate `references/…` mention, so the gate would either flood false positives or need a hand-maintained allowlist — the same failure mode as the forbidden-word scanner rejected above, one level up.
- **Adopted:** narrow mechanical checks where the question IS unambiguous (3a: a `node scripts/x` instruction inside a GENERATED artifact must resolve in the generated project — an executable claim with a binary answer), plus the standing audit obligation in `AGENTS.md` § Reference-closure check and `SKILL.md` § Audit 流程 step 3, whose primary evidence is tarball → INIT → resolve-there.
- **Revisit when** the same defect class escapes three more times despite the obligation, or when a mechanical way to separate "executable instruction" from "provenance mention" appears (e.g. a required syntax for runnable commands in generated artifacts). Then the gate becomes cheap and the objection above dissolves.
...
**Adopted decision (corrected from the first draft).** The evaluation's core proposal is right, and this section now states the full split — removing the repo-specific content from `release.md` rather than keeping it "as-is with caveats". The first draft's "release.md keeps its governed-project content and NOT its repo-specific lines" left the document self-contradictory: `release.md:38` (the validator exemption for this repo) and `release.md:183,198` (generator defaults, `package-skill.sh` packaging) are THIS REPO's content and do not belong in a governed-project release policy. If they stay, `release.md` remains "target-project flow + this-repo exceptions", the exact mixing this plan exists to end.

**Reset target shape:**

- `release.md` = **Governed Project Release only**. Remove its skill-repo self-exemptions and path mapping; every "this repo" clause moves to `skill-release.md`. `validator.passed` keeps the governed-project requirement (`verify_governance.js` exit 0; a target that truly lacks it is a different mistake than this repo's ADR-0006 exemption); `docs.parity_passed` keeps its conditional; `plan.delivery_verified` keeps its conditional (N8 fix from §1.5). The skill-repo exemption for a target-structure the target HAS is also removed — its only legitimate home is the skill repo's own release doc.
- `skill-release.md` (NEW, SKILL-INTERNAL) = **Skill Repository Release** — skill version sources (SKILL.md frontmatter, package.json, CHANGELOG, init-spec default, generator fallback, tag), skill tests, tarball build, tarball content whitelist, checksum, GitHub Release, user approval. It also carries wherever `release.md` had a "this repo" clause, so the two documents split five ways clean.
- `check:release` — **the split is NOT optional.** Naming confusion has already happened (it is in this repo's package.json, runs this-repo tests/trilingual docs/plan delivery/archive/translation/CHANGELOG, and a governed project has none of those). Decide: `check:skill-release` = skill artifact check (tests + payload + packaging boundary); `check:repo-release` = repo maintenance/archive checks. Keep `check:release` only as a documented repo-only compatibility alias, if kept at all. The split lands with the same change set as `skill-release.md`.
- `plan/archive/roadmap` = **Repo Maintenance** — continue running, but never described as "skill artifact validity proof".

**Deliberately NOT abstracting a shared workflow.** (unchanged) Shared BEHAVIOR stays in scripts; the two documents each describe their own flow.

**C6 stays deferred.** (unchanged)
...
- `references/workflows/release.md` — §1.5 (hard gate L41) + N16 (release_requirements duplicated with sub-skills.md, already drifted) + §4 — remove its skill-repo self-exemptions (L38, L183, L198) and path mapping, which move to skill-release.md
- `references/workflows/skill-release.md` — **NEW (SKILL-INTERNAL)** — §4 Skill Repository Release: skill version sources, skill tests, tarball build + content whitelist, checksum, GitHub Release, user approval; carries every "this repo" clause removed from release.md. NOT in init-spec as an artifact (it is SKILL-INTERNAL, not INSTALLED), does not enter a governed target, ships in the tarball.
- `references/templates/sub-skills.md` — (already listed above) §1.2 + §4 — the governing-project release-manager sub-skill no longer cites `references/workflows/release.md` as its authority; it cites the installed target flow and keeps its own 11-key requirements
...
- `references/workflows/ci.md` — **N19: adjudicated as NOT a defect, no change made.** Measured against `references/init-spec.json`: `.github/workflows/ci.yml` is a **Phase B** artifact and `scripts/verify-governance.js` is installed in **Phase B** as well, so the generated workflow never references a script its own stage has not installed. The stage-closure rule is satisfied; the file is listed here only to record that it was checked.
...
- `tests/suites/payload.test.js` — §3a generated-references-resolve (all sub-skills + rules) + N20's three phase-contract regressions (Phase A references no uninstalled script · Phase A is marked initialization-incomplete · Phase B/C carry the requirements for the scripts that stage installs) + §3c dead-link sweep (payload fixture, NOT check-doc-consistency.js)
- `tests/suites/docs.test.js` — §3b check-layout-sync no-op
