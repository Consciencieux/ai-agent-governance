...
#### C6 評審證據綁定 —— 延後的決策，不是遺忘

**問題。** `release-manager.js execute` 對高風險發佈的把關，依據是提案裡的
`reviewStatus` 等於 `completed` 或 `explicitly-approved`。該值讀自呼叫方提供的
提案 JSON。`plan` 從不產出這兩個值（它只產出 `required` / `suggested` /
`not-required`），所以走到該分支的提案必然經人手編輯——這正是 human-in-the-loop 的
預期形態，但也意味著這道閘門校驗的是一句**聲明**，而非「評審發生過」。稽核已實測：
手寫一份 `reviewStatus: "completed"` 的提案即可產出帶簽名的 annotated tag，退出碼 0，
全程沒有任何評審。對照同一函式裡的 `headSha` 綁定——它真的執行 `git rev-parse HEAD`
並在漂移時中止。

**當前風險。** 低到中等，受威脅模型限制：行為者必須已有寫入權限、手寫 JSON、並傳入
`--yes`。這是「Agent 抄近路」的自證漏洞，不是外部攻擊面。殘餘危害是一個斷言「已評審」
而評審從未發生的簽名標籤。

**本輪不做的原因。** 真正的綁定需要兩樣尚不存在的東西：評審證據工件（review-manager
產出、綁定到具體 commit 的記錄）與 `execute` 據以核對的工作流。兩者都是新機制，機制
測試不通過：本變更集的需求是修復失效閘門，該需求不足以獨立證成「發明一套證據格式加一
條校驗流程」。本輪做的是誠實最小集——`execute` 現在明確列印該值為自證聲明，簽名標籤
不再隱含「已評審」。

**不採用提案摘要簽名的原因。** 稽核給的更廉價方案是讓 `plan` 對提案簽名或雜湊，使
`execute` 能識別手寫檔案。否決：它偵測的對象不對。手動編輯提案正是**已文件化的批准
路徑**——開發者在逐項確認後寫入 `explicitly-approved` 是在遵循流程，不是在繞過它。
摘要簽名會把這條合規路徑變成閘門失敗，同時仍然無法證明評審是否發生。它買到的是「檔案
來自 `plan`」，而這不是本問題要證明的性質。

**重新評估的觸發條件**（任一成立）：發佈批准需要證明**評審者身分**；評審證據必須綁定
到具體 commit；合規要求高風險發佈留存稽核軌跡；或 review-manager 因其他原因開始產出
持久工件（屆時校驗側的成本很低）。在此之前維持誠實聲明。觸發時建立獨立 TASK 計劃——
不要把它塞進無關的變更集。
