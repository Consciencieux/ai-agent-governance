...
| **SKILL-INTERNAL（隨 tarball 但不安裝）** | 隨 tarball 分發（打包整目錄複製 `references/` + `scripts/`）且由 **skill 執行器**讀取——但 INIT 從不安裝它，所以被治理專案裡沒有這個檔案。 | 在 `init-spec.json` 的 `distribution.skillInternal` 中列出 | `references/workflows/release.md`、`references/init-spec.json`、`scripts/generate-governance.js`、`scripts/package-skill.sh`，以及僅本倉庫執行的閘門（`check-doc-parity`、`check-layout-sync`、`check-plan-delivery`、`check-coding-hygiene`、`check-role-completeness`） |
