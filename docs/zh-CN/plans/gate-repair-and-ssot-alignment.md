...
- **C6 评审证据绑定 —— 本次只做诚实化，实质绑定延后。** `execute` 的 `reviewStatus` 是调用方提供的字符串，`plan` 从不产出 `completed`，所以走到该分支的提案必然是手写的——这正是 human-in-the-loop 的预期形态，但它校验的是**声明**而非"评审发生过"。本次仅让 `execute` 明确打印该值为自证声明（不再让签名标签隐含"已评审"）。真正的绑定需要评审证据工件 + 评审工作流，属于新机制，当前需求不足以独立证成，另案处理。
