...
## Script Inventory 说明（FINDING-0028）

# Script Inventory v0（施工权威 · FINDING-0028）

机读源：[`repo-tools/script-inventory.v0.json`](../../repo-tools/script-inventory.v0.json)  
基线戳：`v1.0.2`（证据列，**不是**隔离轴）  
施工计划：[PLAN-0041](../plans/archive/PLAN-0041-script-inventory.md)
...
H1（PLAN-0037 / ADR-0025 决策 9–10）**只消费本台账，不另建能力去向表**。H1 期间不得把 `keep` 预标为 `retire`，不得整夹隔离。第三类（无 2.x 价值）今日是空集；真正的 `retire` 只在 H2a 写入本 JSON 之后才允许停用引用。
