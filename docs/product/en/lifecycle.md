...
The full lifecycle also includes Change Hygiene (current/compatibility/history surface checks for high-impact changes), Rule Capture (explicit adjudication before persistent developer requirements enter the rule files), and the Discovery Ledger (known-issue closure workset on medium/large TASK plans — ADR-0021); all are defined in the same policy.
