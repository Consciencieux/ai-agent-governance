...
**1. 结构层（系统为何只能靠加长文档）**  
Prompt/文档是被动承载、常被全量或大块注入，缺少按决策点的适用性路由与机械 carrier。Generation-1 用文档体系承担本应由控制平面承担的路由与适用性（`Agent memory = dispatcher`）。没有图状适用关系、关键保证仍靠「记得读某段 Markdown」时，唯一可操作的旋钮就变成：**把规则写进更长的静态文，以提高「存在率」**。规则总量与注意力稀释成正比。

**2. 生成层（单次编辑为何爱把一句话写成一页）**  
文档过长不只是「历史堆出来的」，还有稳定的**写作替代策略**：

| 压力 | Agent（与人）的常见反应 | 结果 |
| --- | --- | --- |
| 无法机械 enforce | 用更长散文「说清楚」冒充控制 | guidance 膨胀，control 仍缺 |
| 不确定权威边界 | 预写例外、背景、复述、清单，防「你漏了 X」 | 一句可立的规则变成自洽小百科 |
| 单文件自足偏好 | 少用指针，把权威正文抄进入口/索引 | 多源漂移 + always-on 变厚 |
| 「完整 = 负责」的训练偏好 | 把概括当成偷懒，把展开当成质量 | 可一行的指针被写成段落 |

本质一句话：**在缺少机械控制与明确「指针即完成」纪律时，加长自然语言是对不确定性与不可执行性的补偿行为**——不是信息真的需要那么多，而是系统只奖励「写在纸上的存在感」。

因此：只拆文件、不改路由/carrier，只会把一个大 prompt 变成许多小 prompt；只骂「废话多」不补控制平面，会反复复发。
