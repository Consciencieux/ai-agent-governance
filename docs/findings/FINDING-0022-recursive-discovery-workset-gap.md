...
# 修复过程中已发现问题可能因 recursive discovery 丢失（缺持久 workset）
...
（未解决，remediation underway。）ADR-0021（Known-Issue Closure 执行语义）与 PLAN-0033（第一代 Discovery Ledger 实现）已建立；正式纳入 TASK Plan 格式与 lifecycle.policy 留待后续阶段。
