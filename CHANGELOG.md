...
- **Issue / Finding archive** (`docs/findings/`) — repo-native, version-controlled archive for research observations and failure modes. 简体中文 single-language, never archived (status changes in place). Lifecycle: `Proposed / Confirmed / Resolved / Superseded / Invalidated` (no Open/Closed); `severity` separated from `status`; `type` binds L0–L4 (defect / mechanism-gap / control-gap / architecture-gap / research-observation). Systematic findings must be promoted to findings, not closed as patches. Schema, taxonomy and lifecycle in `docs/findings/README.md`. Decision: (ADR-0013).

- **Architecture Migration Mode** (ADR-0014) — before the Generation-1 → Generation-2 refactor: legacy gates become observational, a minimal Refactor Safety Kernel stays authoritative, validation moves to architecture checkpoints, release is forbidden, and rule evolution is frozen (systemic issues → `docs/findings/`, decisions → ADR, not new 1.x mechanisms).

- **Research knowledge base** (`docs/research/`) — a fourth knowledge class alongside findings/ADR/plans: neutral system characterization (baseline, system model, mechanism taxonomy, evaluation framework) vs findings (problem judgment). 简体中文 single-language, not trilingual; lifecycle is version evolution (`Draft / Active / Superseded / Archived`), never deleted — superseded models are research trajectory. Seeded with RESEARCH-0001 system-model (Generation-1 control model), RESEARCH-0002 mechanism taxonomy (6 gate mechanism classes), RESEARCH-0003 evaluation framework, RESEARCH-0004 architecture evolution (Generation 0→3).

- **Archived plans given stable IDs** — 30 archived plans in `docs/archive/` renamed to `PLAN-0001..0030-<slug>.md` (IDs assigned by git creation order, permanently frozen); roadmap links, CHANGELOG pointers and the INSTALLED `lifecycle.policy.md` reference updated to the IDs.

- **Roadmap repositioning decision** (ADR-0015) — roadmap is redefined from a feature-inventory-plus-roadmap hybrid to an architecture-evolution view (Vision / Current State / Known Limitations / Target Architecture / Migration Phases / Research Goals / Non-goals); completed capabilities move to `docs/research/RESEARCH-0005-current-capabilities.md`; maintenance trigger changes from every-release to architecture events. Implementation follows as a separate change.

- **Purpose-first doc structure decision** (ADR-0016) — language is no longer the first classification axis; only user-facing docs stay trilingual (`docs/product/{en,zh-CN,zh-TW}/`), while plans/findings/research/ADR/archive are single-language by type. Roadmap stays trilingual as a boundary object. Implementation is phased under ADR-0014 Migration Mode (gate-script path hardcoding and test fixtures updated per phase), not a one-shot move.
