...
- **Sibling-instance closure and control-plane tracing in the root-cause repair protocol** (`references/policies/lifecycle.policy.md`, INSTALLED) — two failure shapes were left open by the two-domain clause alone: fixing one instance without searching the other surfaces that share the same contract, and fixing a visible output without tracing the rule/template/generator that produced it or the gate that failed to catch it (the next generation overwrites such a fix). The protocol now requires enumerating sibling instances with the search scope and command stated — "found nothing else" is not evidence — and tracing the relevant control plane (authoritative rule → template/generator → output → checker/gate → test oracle → release or target consumer), with a short outcome per surface and layer: fixed / correct with evidence / not applicable with reason / blocked. The requirement reaches the executing side too: the generated `AGENTS.md` Phase 3 carries an operational summary, and AUDIT's "minimal patch" step now states that minimal means not widening the change, not stopping at the reported line. No similarity engine, no audit registry, no mandatory five-layer table, no new gate — existing repair sessions, impact-face search and change-hygiene reconciliation carry it.

- **Two-domain symmetry in the root-cause repair protocol** (`references/policies/lifecycle.policy.md`, INSTALLED) — a governance defect usually lives in two domains at once: the side that authors the rule and the side that executes it. Fixing only the reported side and assuming the other is correct produced four real misses (the v1.0.1 provenance trap shipped the same bug to every governed project; five-vs-seven version sync points; a sentinel gate that never matched; `AGENTS.md` hardcoding npm commands regardless of stack). The protocol now requires checking the corresponding domain — artifact defects must trace back to the template/generator that produced them, rule defects must check their projections — and stating a reason when the other domain does not exist or does not apply, instead of assuming correctness. When a gate claims to intercept the defect, one bounded negative check must prove it is not idling (reinject → must fail; restore → must pass). Deliberately narrowed to this one clause: defect-signature systems, control-surface review tables, instance-outcome taxonomies, similarity search and new gates are explicitly excluded, and the exclusion is written into the rule text so it cannot be quietly widened.

- **Domain-level test entry: `node tests/run-tests.js --suite <name>` and `--list`** — the test split (11 domain suites) landed without the domain-level runnable entry that anti-patch-development §3 promised, so the dev loop could only use the full entry: editing one README punctuation mark demanded a 42.9s full run (96% of `npm run check`'s 44.6s). A single suite now runs on its own (hygiene: 0.4s). Suite names are canonical (`docs`, not `docs.test.js` — a file-name alias is rejected so a second naming rule cannot appear) and `--list` prints them in `SUITES` declaration order, doubling as the visible registration order. `--suite all` resolves to the same array the no-arg path uses, so exactly one full-run code path exists. Deliberately manual: `npm test` and `npm run check` stay full, no gate selects a subset, and nothing infers a suite from a diff — full regression before release/audit/commit is unchanged.
...
- **The empty `[Unreleased]` rebuild-timing mistake now carries a mechanical diagnosis** — skill-release.md step 2 warns the empty section must be rebuilt AFTER the release gates pass (v0.15.0 and v1.0.1 both hit it), but an operator who rebuilds it early only saw a bare changelog-coverage failure with no hint of the cause. The `changelogCoverage` cluster now detects an empty `[Unreleased]` section at release time and reports "likely rebuilt too early" (fail-closed at `--release-gate` only; a freshly rebuilt empty section is the normal post-release state and stays advisory in daily `--gate` mode, both pinned by tests).

- **skill-release.md step 9 clarified that the release Proposal must be regenerated by re-running `release-manager plan`, not hand-edited** — the step said "update `headSha` to the new HEAD", which reads as an invitation to edit the JSON; `execute` recomputes the provenance binding over the risk/review/version/headSha fields and rejects a hand-edited proposal (v1.0.1 hit exactly this mid-release). The step now states that the proposal is regenerated after the release commit so `headSha` and `provenance` are both rebuilt consistently.

- **The same provenance trap was live in the governed-project release flow and the generated release-manager sub-skill** — `references/workflows/release.md` step 7 and `references/templates/sub-skills.md` step 7 both told the operator to "update `headSha` to the new HEAD" by hand, while the INSTALLED `release-manager.js` `execute` recomputes `provenance` (bound to headSha among other fields) and rejects any edited proposal. Every governed project releasing with a post-release-commit proposal would hit "proposal provenance does not match". Both documents now state that the proposal is regenerated via `plan` after the release commit and never hand-edited (v1.0.1 release taught this in this repo; the governed-project copies carried the same bug).
...
- **Governance lessons migrated to the payload: declaration-mechanism gap, evidence tiers, test activity, CI gate completeness** — this change set writes 0.13.0+ lessons into the INSTALLED rules: `lifecycle.policy.md` Phase 4 now carries a three-tier evidence classification (mechanical/human-attested/unverified) and a new § 声明与机制的差距; `testing.policy.md` gains a § 测试活性 with vacuous-test definition, fact-source rule, and mutation priority tiers; `coding.policy.md` adds an enumeration re-check item to change placement; `ci.md` adds a CI gate completeness note with degradation-convention alignment; the AGENTS.md template carries a declaration-mechanism consistency pointer in the Definition of Done section. All rules are marked as human-attested/unverified — no new gate scripts, no mechanical enforcement claimed.
...
- **Six low-severity issues from the whole-project audit closed** — (1) a test skip printed `(skipped: ...)` and returned `true`, so the summary counted it as covered while nothing was asserted: the runner now recognises a `skip:` string return and reports it as skipped, not passed. (2) The four plan-status classifiers (doc-consistency, plan-sync, roadmap-sync, plan-delivery) diverged on the same file — a Status line with the colon outside the bold was design-only in one and unknown in another: all four now use the canonical `> **Status: <keyword>**` line form. (3) The layout gate was one-directional — a documented tree entry whose file was deleted stayed in `docs/` forever with a green gate; it now also reports stale entries (documented under a scanned root but gone), and the parser handles box-drawing depth and multi-file branch lines so real and fixture trees both parse. (4) The protected-list completeness check used `file.includes(p)`, so a path mentioned in later prose satisfied an enumeration deletion: it is now scoped to the enumeration block. (5) `isDesignOnly` in the delivery gate anchored its Status regex with `^` without the `m` flag, so a Status line that was not the very first line was never recognised; the canonical multi-line form now matches. (6) The layout-sync message claimed "36 files under references/ + scripts/" while scanning four roots; the wording now matches reality.

- **CHANGELOG structure contract is now part of the INSTALLED rules** — the v0.14.1 record carried three `### Fixed` headings in one section while every gate stayed green: governed projects had a when-to-write rule but no how-to-structure rule. `references/policies/lifecycle.policy.md` and `references/templates/agents-md.template.md` now carry it: one category heading per version section at most, append inside the existing block, never touch a published section, no empty category heading, rebuild empty `[Unreleased]` after renaming, canonical category names only. `check-doc-consistency.js` now fails closed on a repeated category heading in both `--gate` and `--release-gate` (a structure defect, unlike a merely missing record, which blocks only at release).
...
- **Roadmap drifted from the plans it indexes, and nothing noticed** — `plan-delivery-anchors` was implemented and the roadmap Done list never learned about it. The rule ("re-baseline roadmap horizons at each release") was followed as written and still missed it: it covers the release moment, while plan-lifecycle events (created / implemented / archived) happen between releases with no mechanical coverage — the same shape as the release sync points documented in five places and verified in two. Three fixes: (1) `AGENTS.md` now states that the roadmap is an INDEX and the design plans are the fact source — index entries link their plan and never restate its detail, and sync happens at the lifecycle event, not only at release; (2) a new REPO-ONLY gate `repo-tools/check-roadmap-sync.js` verifies exactly three decidable relations (implemented plan appears in Done · archived plan is not linked from an active horizon · an active-horizon entry naming a plan links it), wired into `check` / `check:docs` / `check:skill-release`; two false-positive traps shaped its matching rules — the substring "design plan, not implemented" contains "implemented", and a slug mentioned in prose is not an index entry; (3) all three roadmap trees now link their plans or say *no design plan yet*. Absence from the roadmap stays legitimate — the gate checks consistency, it does not invent entries.
...
- **The published v0.13.1 tarball carried 34 AppleDouble junk files** — packaged on macOS, where `cp -R` preserves extended attributes and `tar` then writes them as `._name` members: one shadow per payload file (`._SKILL.md`, `scripts/._generate-governance.js`, …), doubling the entry count from 34 to 68 and littering every user's unpacked directory. No governance boundary was crossed (no shadow corresponded to a repo-only file), but the asset users download was polluted. `repo-tools/package-skill.sh` now suppresses the metadata at every layer it can — the `COPYFILE_DISABLE` / `COPY_EXTENDED_ATTRIBUTES_DISABLE` environment, a probed `cp -X`, probed `tar --no-xattrs/--no-mac-metadata` flags, and a `find -delete` sweep of the staging tree — then **fails closed**: it greps the finished archive for `._*` / `.DS_Store`, and if any survived it deletes the tarball and exits non-zero rather than publishing it. The flags a platform honours vary, so the post-build check, not the flags, is what guarantees the result.
...
- **Review of the enumeration fixes found three of the new regression tests were themselves defective** — (1) the "identifier wired into repo-tools/" test declared its identifier in the plan's Validation Method section, which the checker treats as narrative: the test could pass even when the identifier was never wired anywhere. (2) The packaging mutation test rewrote the real `repo-tools/package-skill.sh` in place and shared the real `dist/` — a killed process would leave a weakened protected file behind; it now runs against an isolated tmp mini-repo. (3) The hygiene and plan-delivery pins enumerated subsets of their target lists, so dropping `repo-workflows/` from the scan or the four root files from `SEARCH_ROOTS` stayed green (both pins now assert the complete sets, and the residue assertion checks the marker category, not a bare substring). Also closed in the same pass: `repo-workflows/` joined the hygiene scan (its own comment had claimed "every source tree" while excluding it), the checker's failure message is derived from `SEARCH_ROOTS` instead of a stale hardcoded list, `repo-tools/*.sh` joined the protected-files summary, two not-yet-implemented design plans were updated to the post-split paths, and AGENTS.md now carries the standing rule this defect class needed: after any file move, re-check every hardcoded directory enumeration — three separate gates have gone blind this way, each passing green until an enumeration test was added.
...
- **Release flow documentation audit fixes** — (1) AGENTS.md told maintainers to run `npm run check:all` before release; its `--release-gate` clusters stay advisory there, so the release gate is now correctly named as `npm run check:skill-release` (audit-before-release uses check:all), and the gate-location pointer was redirected from `release.md` (governed projects) to `repo-workflows/skill-release.md` Phase 4 step 3 (this repo's flow). (2) The `references/` release list claim (five places incl. SKILL.md frontmatter) was corrected to four — generated sub-skill SKILL.md files carry no `version` field, verified; SKILL.md:239 and the generated requirement already said four. (3) `repo-tools/*.js` header comments that still said "ships inside the payload tarball" / "`node scripts/`" and the architecture "One nuance" exception sentence (contradicting the same file's "never distributed" label) were rewritten to the post-split facts. (4) Duplicate `"check:release"` key in package.json removed (RFC 8259). (5) Protected-files summary now covers `repo-tools/*.js` and `repo-workflows/**`. (6) Test count claim generalized to "all tests registered by tests/suites/*.test.js".

- **v0.13.1 post-release repair: the CHANGELOG version section was never advanced** — the v0.13.1 release shipped (tag, GitHub Release, tarball) with all 40 entries still under `[Unreleased]` and no `[0.13.1]` section, and every gate stayed green: `changelog_coverage` only asks "is the change recorded", never "has the version section advanced", and `version_examples` explicitly excluded CHANGELOG.md. Two-part fix: (1) the section is now repaired — entries moved into `[0.13.1] - 2026-09-05` and an empty `[Unreleased]` rebuilt at the top; (2) the `version_examples` cluster gained a mechanical backstop: the newest versioned section in CHANGELOG.md must equal the current `package.json` version (fail-closed under `--gate`/`--release-gate`), mutation-verified by a test reproducing the exact incident shape. Two pre-existing coverage fixtures had their `package.json` versions aligned with their changelog sections — they tested coverage semantics and their version mismatch was an unrealistic artifact of fixture construction.
