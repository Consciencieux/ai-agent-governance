...
- **1.0.0 freezes the public interface surface.** (ADR-0011) — five categories are now stable and change only under SemVer: sub-skill trigger words, the INSTALLED script CLIs, the INIT contract, the governed-project rule paths, and the `.governance/` runtime contract. Breaking changes require MAJOR plus migration guidance. Each category's current members stay in its own source file; ADR-0011 records why the categories are protected and how breaking changes are handled.

### Fixed

- **The generator's fallback-sentinel sync point had a vacuous backstop** — the `version_examples` cluster matched `fallback: "X.Y.Z"`, but the sentinel is the else-branch of a ternary, so the pattern could never match it. The check had claimed to cover this sync point since v0.13.2 while covering nothing: a release could ship a stale sentinel — the version stamped into every future INIT — with every gate green. The pattern now anchors on the ternary.

- **The release flow did not say that documentation version EXAMPLES also block the gate** — the flow listed the five sync points, but the same `version_examples` cluster fail-closes on any `"version": "X.Y.Z"` example in a `.md` file, which stopped a release mid-sequence. The step now names examples separately from the sync points and points at the gate as the way to enumerate them instead of relying on memory. It also records that the empty `[Unreleased]` section must be rebuilt after the gates pass, not during version sync.
...
- **The release flow is split by audience** (plan: repository-boundary-split) — one document carried three intents (release this skill · release a governed project · repo maintenance), and the seams showed as self-exemptions inside a policy meant for targets. `release.md` is now Governed Project Release only, and the new SKILL-INTERNAL `references/workflows/skill-release.md` carries this repo's own flow, including the ADR-0006 statement that `verify_governance.js` is expected to exit 1 here and must not be satisfied by fabricating a `.governance/`. `npm run check:release` is now an alias for `check:skill-release`.
