...
- **Phase 6 Report**: modified files, new features, deleted/renamed/replaced/deprecated content, Change Hygiene Ledger results, validation results and doc updates. Compare actual changed files against the planned Affected Files list — listed-but-unchanged → fix or justify; changed-but-not-listed → explain.
...
Content boundary (what belongs in an entry): record what changed → what impact it has on users/maintainers → migration guidance if needed. Unit: one entry = one change (one fix, one capability, one rule or process adjustment, one doc rewrite). Merging several independent changes into one entry is a violation — judge by whether one reader can consume a single sentence and know its impact; if not, split. Do NOT record: test commands, gate output, mutation-verification, INIT internals, audit narration, or evidence logs — those belong in plan acceptance, review reports, or release proposals. Leave adjudication elsewhere: root cause, alternatives, constraints and background belong in design plans, archived plans or ADRs — an entry may carry a pointer (`(plan: name)` / `(ADR-NNNN)`) by NAME, never by path, since plan paths change on archival but names do not. Cite rules rather than restating them: when an INSTALLED rule changes, record "the rule now requires X — see <rule file>", never copy X's full definition into the change record. Standard-entry examples live in the rule file (`docs/rules/lifecycle.md` § CHANGELOG 内容边界) — model new entries on that canonical example, not on the previous entry, which may have drifted.
