...
**确认凭证卫生：** 事后记录的 staged digest / commit hash **不等于**「人已理解变更」；不得用 hash 冒充理解或替代人授。若项目使用 `.governance/change-hygiene.json`（或等价）声明卫生字段，字段须与该变更集绑定，缺项不得宣称卫生已闭合。
...
受保护路径清单的**单一事实源**是 `docs/rules/governance-files.md`（完整清单见该文件；本 skill 源：`references/policies/governance-files.policy.md`）。本文件**不**维护第二份路径表。

修改清单内文件：说明原因 → 更新 CHANGELOG → **更新 `.governance/manifest.json` 的 `governance_version`** → 运行 `scripts/verify-governance.js`。
