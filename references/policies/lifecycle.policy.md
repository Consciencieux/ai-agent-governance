...
**规模分级只决定"要不要写 TASK 计划文档"，不决定"要不要给用户确认提交"**——明确写指令或 IDE 暂存+提交+推送确认条即是该变更集人授，覆盖 add → commit → push，对所有规模一视同仁（见 Git Write Policy 一次确认 per 变更集）。判定顺序：先看"是否破坏公共接口/API/数据结构"→ 再看"是否跨模块/涉及功能"→ 最后看改动量。边界模糊时**取更高级别**（宁完整不省略）。
