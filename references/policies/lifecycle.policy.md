...
- **双域对称（治理缺陷必查另一域）**：治理缺陷往往同时活在两个域——编写规则的一侧和执行规则的一侧。只修复被报告的那一侧、默认另一侧正确，是已被多次证实的漏修模式。适用对象限定为治理规则、模板、生成器、门禁、测试、生成产物与发布流程；普通业务代码与孤立措辞错误不触发。
  ...
  - 本条只要求"检查另一域并给出结论"，不引入缺陷签名体系、控制面审查表、实例结局分类或通用相似度搜索。
