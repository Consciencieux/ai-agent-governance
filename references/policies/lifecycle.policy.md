...
| 规模 | 判定 | 流程 |
| --- | --- | --- |
| **小** | 单关注点、不破坏公共接口/API/数据结构；可为少量相关文件（含 typo/文档/格式/局部修复）；不新建产品能力、不改规则契约 | 简化路径：Understand → Implement → Validate → Report（跳过 Plan 与 Synchronize；机械检查兜底） |
| **中** | 触及功能/模块/规则契约，或需要跨模块协作交付，但不破坏公共接口 | 完整六阶段 + TASK 计划 |
| **大** | 公共接口/API/数据结构变更、跨模块重构、架构变化 | 完整六阶段 + TASK 计划 + 审查（由 `review-manager` 子技能承担） |

**多文件本身不自动升为「中」。** 同一关注点下的配套改动（测试+实现、文案+链接）仍可判「小」。

**规模分级只决定"要不要写 TASK 计划文档"，不决定"要不要给用户确认提交"**——明确写指令或 IDE 暂存+提交+推送确认条即是该变更集人授，覆盖 add → commit → push，对所有规模一视同仁（见 Git Write Policy 一次确认 per 变更集）。判定顺序：先看"是否破坏公共接口/API/数据结构"→ 再看"是否触及功能/规则契约或跨模块协作"→ 最后看是否仍为单关注点局部改动。边界模糊时**取更高级别**（宁完整不省略）。

## Phase 1 — Understand（理解）

开始前按**任务相关路径按需读取**（未读与任务相关的权威内容前，不得进入 Phase 2/Implement，也不得声称已理解该部分）：

- `AGENTS.md`（入口路由与本次任务相关节）
- 任务触及的 `docs/ARCHITECTURE.md` 相关节、相关 `docs/features/` 条目（有则读；无相关条目则不扫全目录）
- 若改动依赖近期已发布行为或可能冲突的变更，再读 `CHANGELOG.md` 最近相关条目

**禁止**为「仪式完整」通读无关 Feature 全目录或整份 CHANGELOG。理解不是"四个路径都打开过"，是能说清「要动的部分在哪、碰了会影响谁」。
...
- **发现台账（Discovery Ledger）**：仅当本任务是 **bug 修复或机制变更**（规则/脚本/门禁/生成器/模板）时，中/大型 TASK **必须**携带本节（见下方同名专节）。其他中/大型任务按需启用；小型改动默认跳过。若执行中已登记条目，完成前仍须闭包。

中/大型 **bug 修复或机制变更**任务还必须包含「根因修复协议与失败预算」的字段与先决条件（见 Phase 3 同名小节）。

**小型**改动（见"规模分级"）跳过本阶段，直接进入 Implement；报告中一句话说明规模判定即可，无需逐条理由。

**用户确认门**：中/大型的 TASK 计划创建后，**必须展示给用户确认**（展示 Proposed Solution、Affected Files、Risks、Validation Method）。这是**意图对齐**，不是提交授权——改什么、怎么改对齐了，提交前仍须走一次确认（见 Git Write Policy 一次确认 per 变更集）。未获确认不得开始实现（除非用户明确豁免）。文件数量本身不单独构成确认门。
...
Phase 2：**bug 修复或机制变更**的中/大型 TASK 必须携带台账；其他中/大型按需；小型默认跳过。若执行中已登记条目，完成前仍须闭包。
